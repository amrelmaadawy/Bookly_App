...
class homeRepoImpl implements HomeRepo {
  ...
  Future<Either<failure, List<BookModle>>> fetchFeatureBooks() async {
    try {
      var data = await apiService.get(
          endPoint: 'volumes?q=computer scince &filtering=free-ebooks');
      List<BookModle> books = [];
      for (var item in data['items']) {
        books.add(BookModle.fromJson(item));
      }
  ...
  @override
  Future<Either<failure, List<BookModle>>> fetchSearchBooks({required String quary}) async{
    try {
      var data = await apiService.get(
          endPoint: 'volumes?Filtering=free-ebooks&q=$quary &Sorting=relevance');
      List<BookModle> books = [];
      for (var item in data['items']) {
        books.add(BookModle.fromJson(item));
      }
